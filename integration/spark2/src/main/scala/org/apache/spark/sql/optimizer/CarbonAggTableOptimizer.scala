...
import org.apache.spark.sql.hive.{CarbonMetastore, HiveSessionCatalog}
...
object CarbonAggTableOptimizer {
  ...
  var aggTableList: List[CarbonTable] = List()
    ...
    if (enableUseAggTable && needToTransformtoAggTable(plan)) {
      ...
      aggTablePlan = plan transform  {
          case agg@Aggregate(groupbyExp, aggregateExp,
            ...
            rel@LogicalRelation(relation, _, _))))
            ...
            aggTableList.foreach { aggTable =>
              // first need to collect the attributeReference of agg table
              // for spark-1.6 need to change this
              val tableIdentifier = TableIdentifier(aggTable.getFactTableName,
                Some(aggTable.getDatabaseName))
              val aggRelation = sQLContext.sparkSession.
                catalog.asInstanceOf[HiveSessionCatalog].lookupRelation(tableIdentifier) match {
      ...
      if (isMatch) {
        newAggregateExp = mainAggregateExp.map { exp =>
          exp transform {
            ...
            // for spark-1.6, no need to add package.
            case sum@Sum(_) =>
              val aggFieleName = CarbonMetastore.convertFuncToExpression(sum.prettyJson)
              Sum(aggAttributes.get(aggFieleName.toLowerCase()).get)
            case count@Count(_) =>
              val aggFieleName = CarbonMetastore.convertFuncToExpression(count.prettyJson)
              Sum(aggAttributes.get(aggFieleName.toLowerCase).get)
            case max@Max(_) =>
              val aggFieleName = CarbonMetastore.convertFuncToExpression(max.prettyJson)
              Max(aggAttributes.get(aggFieleName.toLowerCase).get)
            case min@Min(_) =>
              val aggFieleName = CarbonMetastore.convertFuncToExpression(min.prettyJson)
              Min(aggAttributes.get(aggFieleName.toLowerCase).get)
  ...
  def checkMatch(aggTableExp: Seq[Expression], mainTableExp: Seq[Expression]): Boolean = {
    ...
    breakable {
      mainTableExp.foreach { mainExp =>
        ...
        mainExp match {
          case Alias(CaseWhen(branches, _), _) =>
            branches foreach {branch =>
              branch.asInstanceOf[Expression] foreach {
                case sum@Sum(_) =>
    ...
    if (aggExp.semanticEquals(mainExp)) {
      ...
      if (aggExp.getClass.equals(mainExp.getClass)) {
        aggExp match {
          ...
          case Alias(child, _) =>
           if (hasCast(child)) {
             // for decimal that maybe cast
             val mainChild = mainExp.asInstanceOf[Alias].child
             // require that same aggregate function, same arithmetic type and same attributes
             isMatch = child.getClass.equals(mainChild.getClass) &&
               collectArithMeticType(Seq(child)).equals(collectArithMeticType(Seq(mainChild))) &&
               collectAttributeName(Seq(child)).equals(collectAttributeName(Seq(mainChild)))
           } else {
             isMatch = child.prettyJson.split("AS")(0).trim
               .equals(mainExp.asInstanceOf[Alias].child.prettyJson.split("AS")(0).trim)
           }
          case other =>
        }
        ...
        aggExp match {
          case Alias(child, _) =>
            if (hasCast(child)) {
              // usually for decimal.
              var newChild = child
              // for spark-1.6 need to add this part.
//              child match {
//                case AggregateExpression(fun, _, _) =>
//                  newChild = fun
//                case other =>
//              }
              isMatch = newChild.getClass.equals(mainExp.getClass) &&
                collectArithMeticType(Seq(newChild)).equals(collectArithMeticType(Seq(mainExp))) &&
                collectAttributeName(Seq(newChild)).equals(collectAttributeName(Seq(mainExp)))
            } else {
              isMatch = child.prettyJson.split("AS")(0).trim.equals(
                mainExp.prettyJson.split("AS")(0).trim)
            }
          case other => other
        }
        ...
        mainExp match {
          case Alias(child, _) =>
            isMatch = child.prettyJson.split("AS")(0).trim.equals(
              aggExp.prettyJson.split("AS")(0).trim)
          case other => other
        }
      }
    }
  ...
  def needToTransformtoAggTable(plan: LogicalPlan): Boolean = {
    ...
    var isLoadingIntoAggTable = false
    plan collect {
      case l: LogicalRelation if (l.relation
        .isInstanceOf[CarbonDatasourceHadoopRelation] &&
        l.relation.asInstanceOf[CarbonDatasourceHadoopRelation].segmentId.isEmpty) =>
        val tableIdentifier = l.relation.asInstanceOf[CarbonDatasourceHadoopRelation].absIdentifier
        val mainTable = org.apache.carbondata.core.carbon.metadata.CarbonMetadata.getInstance()
          .getCarbonTable(tableIdentifier.getCarbonTableIdentifier.getTableUniqueName)
        if (!mainTable.getAggregateTablesName.isEmpty) {
          mainTable.getAggregateTablesName.asScala.foreach { aggTableName =>
            val aggTable = org.apache.carbondata.core.carbon.metadata.CarbonMetadata.getInstance()
              .getCarbonTable(tableIdentifier.getCarbonTableIdentifier.getDatabaseName
                + CarbonCommonConstants.UNDERSCORE + aggTableName)
            aggTableList :+= aggTable
          }
          aggTableExists = true
        }
      case agg: Aggregate =>
  ...
  /**
   *  check whether the exprssion has Cast, usually for decimal
   * @param exp
   */
  def hasCast(exp: Expression): Boolean = {
    exp find {
      case Cast(_, _) => true
      case other => false
    } isDefined
  ...
  def collectArithMeticType(expressions: Seq[Expression]): Set[String] = {
    var arithMeticSet: Set[String] = Set()
    expressions.map( exp =>
      exp foreach {
        case Add(_, _) =>
          arithMeticSet += arithmetics(0)
        case Subtract(_, _) =>
          arithMeticSet += arithmetics(1)
        case Multiply(_, _) =>
          arithMeticSet += arithmetics(2)
        case Divide(_, _) =>
          arithMeticSet += arithmetics(3)
        case other =>
      }
    )
    arithMeticSet
