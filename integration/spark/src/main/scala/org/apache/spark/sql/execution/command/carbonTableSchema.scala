...
private[sql] case class CreateAggTable(
  ...
  override def run(sqlContext: SQLContext): Seq[Row] = {
    ...
    val childPlan = queryExecution.analyzed
...
private[sql] case class LoadIntoAggTable(
  ...
  override def run(sqlContext: SQLContext): Seq[Row] = {
    val factTable = org.apache.carbondata.core.carbon.metadata.CarbonMetadata.getInstance()
      .getCarbonTable(dataBaseName + "_" + factTableName)
    val carbonLoadModel = new CarbonLoadModel
    val validAndInvalidSegments = new SegmentStatusManager(factTable.getAbsoluteTableIdentifier)
      .getValidAndInvalidSegments
    val segmentLists = validAndInvalidSegments.getValidSegments.asScala
    ...
    LoadIntoAggTable.loadIntoAggTableBySegment(sqlContext, carbonLoadModel, segmentLists.toList)
